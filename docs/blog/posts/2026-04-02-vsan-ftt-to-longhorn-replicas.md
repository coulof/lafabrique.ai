...
categories:
  ...
  - Longhorn
draft: true
title: "vSAN FTT to Longhorn Replicas : The Rosetta Stone"
...
# vSAN FTT to Longhorn Replicas : The Rosetta Stone
...
**Failures To Tolerate (FTT)** : how many components failures a given object can survive.
!!! info
A component can be a disk, an entire host or a service that stops.

| FTT | Meaning |
|-----|---------|
| FTT=0 | No redundancy. One failure = data unavailable. |
| FTT=1 | Tolerates 1 components failure. |
| FTT=2 | Tolerates 2 components failures. |
| FTT=3 | Tolerates 3 components failures (mirroring only, rarely used). |
...
[Longhorn](https://longhorn.io/){target="_blank"} is a distributed block storage system built for Kubernetes. Each volume gets a dedicated **[engine](https://longhorn.io/docs/1.11.1/concepts/#11-the-longhorn-manager-and-the-longhorn-engine){target="_blank"}** (storage controller) and a configurable number of **[replicas](https://longhorn.io/docs/1.11.1/concepts/#23-replicas){target="_blank"}** spread across cluster nodes.
...
Resilience settings are declared in **[Kubernetes StorageClasses](https://kubernetes.io/docs/concepts/storage/storage-classes/){target="_blank"}** and applied per-volume at creation time. The [Longhorn scheduler](https://longhorn.io/docs/1.11.1/nodes-and-volumes/nodes/scheduling/){target="_blank"} places replicas using a priority hierarchy:
...
Anti-affinity is configurable at three levels: **node**, **zone**, and **disk**, each independently set to hard or soft per [StorageClass](https://longhorn.io/docs/1.11.1/references/storage-class-parameters/){target="_blank"}.
...
<!-- TODO : Insert diagram 2 - policy-placement-logic.png -->
<!-- A horizontal comparison of 3 scenarios: -->
<!-- 1. Strict-Local (1 replica, Pod + Data on same node) -->
<!-- 2. Balanced (2 replicas, Anti-Affinity forced to different nodes) -->
<!-- 3. Zone-Aware (3 replicas, distributed across racks/zones via topology labels) -->
![Policy-driven placement logic in Longhorn](../../assets/images/vsan-ftt-longhorn/policy-placement-logic.png)
...
| Rebalancing after node add | [`replicaAutoBalance : "best-effort"`](https://longhorn.io/docs/1.11.1/high-availability/auto-balance-replicas/){target="_blank"} | Longhorn redistributes replicas to new nodes. Only for healthy volumes. |
...
- **No [stretched clusters](https://docs.vmware.com/en/VMware-vSphere/8.0/vsan-stretched-clusters/GUID-1153D0CE-BA79-4B93-A03A-15BF3BF5E86B.html){target="_blank"}.** vSAN can synchronously replicate across two sites with a witness. Longhorn offers [DR volumes](https://longhorn.io/docs/1.11.1/concepts/#33-disaster-recovery-volumes){target="_blank"} (async replication to a backup target), but no native synchronous site-level replication.
...
- **[Data locality](https://longhorn.io/docs/1.11.1/high-availability/data-locality/){target="_blank"}.** `dataLocality : "best-effort"` keeps one replica on the same node as the workload, reducing network I/O. `strict-local` goes further : single replica, co-located, maximum IOPS. vSAN has no equivalent.
...
- **[Topology-aware provisioning](https://longhorn.io/docs/1.11.1/nodes-and-volumes/nodes/topology-aware-provisioning/){target="_blank"}.** `allowedTopologies` + `strictTopology` + `WaitForFirstConsumer` gives fine-grained control over where volumes land relative to their pods. Zone pinning is declarative.
...
This deserves its own section because the gap is significant and the architectures are fundamentally different. This will be subject to another blog.
...
Longhorn [DR volumes](https://longhorn.io/docs/1.11.1/snapshots-and-backups/setup-disaster-recovery-volumes/){target="_blank"} use a different model. A standby volume on a secondary cluster pulls incremental backups from a shared backup target (S3 or NFS). The DR volume stays in read-only standby mode until manually activated during a failover.
...
- [ ] **Enable [`replicaAutoBalance : best-effort`](https://longhorn.io/docs/1.11.1/high-availability/auto-balance-replicas/){target="_blank"}** globally or per StorageClass to handle node additions gracefully.
- [ ] **Configure [backup targets](https://longhorn.io/docs/1.11.1/snapshots-and-backups/backup-and-restore/set-backup-target/){target="_blank"}.** Longhorn's S3/NFS backup replaces vSAN Data Protection snapshots. Set up [recurring backup jobs](https://longhorn.io/docs/1.11.1/snapshots-and-backups/scheduling-backups-and-snapshots/){target="_blank"}.
...
- [ ] **Consider [data locality](https://longhorn.io/docs/1.11.1/high-availability/data-locality/){target="_blank"}** for latency-sensitive workloads. This is a Longhorn feature with no vSAN equivalent.
...
- [Longhorn Concepts and Architecture](https://longhorn.io/docs/1.11.1/concepts/){target="_blank"}
- [StorageClass Parameters Reference](https://longhorn.io/docs/1.11.1/references/storage-class-parameters/){target="_blank"}
- [Replica Scheduling](https://longhorn.io/docs/1.11.1/nodes-and-volumes/nodes/scheduling/){target="_blank"}
- [Auto Balance Replicas](https://longhorn.io/docs/1.11.1/high-availability/auto-balance-replicas/){target="_blank"}
- [Data Locality](https://longhorn.io/docs/1.11.1/high-availability/data-locality/){target="_blank"}
- [Topology-Aware Provisioning](https://longhorn.io/docs/1.11.1/nodes-and-volumes/nodes/topology-aware-provisioning/){target="_blank"}
- [Node Failure Handling](https://longhorn.io/docs/1.11.1/high-availability/node-failure/){target="_blank"}
- [Best Practices](https://longhorn.io/docs/1.11.1/best-practices/){target="_blank"}
